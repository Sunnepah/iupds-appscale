...
class ReadWriteScopedResourceMixin(ScopedResourceMixin, OAuthLibMixin):
    ...
    def __new__(cls, *args, **kwargs):
        provided_scopes = settings._SCOPES
        read_write_scopes = [settings.READ_SCOPE, settings.WRITE_SCOPE]
    ...
    def dispatch(self, request, *args, **kwargs):
        if request.method.upper() in SAFE_HTTP_METHODS:
            self.read_write_scope = settings.READ_SCOPE
        else:
            self.read_write_scope = settings.WRITE_SCOPE
